...
export const DateTime = (props) => {
  ...
  return (
    <div
      className={`custom_input ${dateTimeParentClassName} ${
        float ? "p-float-label" : ""
      }`}
      style={dateTimeParentStyle}
    >
      {labelProps?.text && (
...
export const Calendar = (props) => {
  ...
  return (
    <div
      className={`custom_input ${calendarParentClassName} ${
        float ? "p-float-label" : ""
      }`}
      style={calendarParentStyle}
    >
      {labelProps?.text && (
        <div className={`${labelProps.labelMainClassName || "pb-1"}`}>
          <NormalLabel
      ...
      {floatLabelProps?.text && (
        <label
          className={`custom-label ${floatLabelProps.calendarLabelClassName}`}
          htmlFor={floatLabelProps.id}
        >
          {floatLabelProps.text}
          <span className={floatLabelProps.calendarLabelSpanClassName}>
            {floatLabelProps.spanText}
          </span>
        </label>
      )}
...
export const DateTimeDisplay = (props) => {
  ...
  const displayDateTime = formattedDateTime.replace(
    /(\d+)\/(\d+)\/(\d+), (\d+):(\d+)/,
    "$3-$1-$2 $4:$5"
  );

  const formattedDateTime1 = currentDateTime.toLocaleString("ja-JP", options);
  const displayDateTime1 = formattedDateTime1.replace(
    /(\d+)年(\d+)月(\d+)日,/,
    "$1年$2月$3日 "
  );
  ...
  return (
    <div
      className={`${fontsize} ${bgColor} ${fontWeight} ${parentClass}`}
      id={id}
      style={parentStyle}
    >
